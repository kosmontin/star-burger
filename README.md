...
Настроить бэкенд: в существующем файле `star_burger/.env`,
...
## Обновление проекта
Для обновления проекта достаточно воспользоваться скриптом `rebuild.sh` в папке проекта в среде Linux

В качестве дополнительного параметра скрипт принимает API Token [RollBar](https://rollbar.com/)

```sh
./rebuild.sh [rollbarToken]
```

Данный скрипт обновляет проект, если есть новые коммиты в удаленном репозитории.
А так же если нет несохраненных локальных.
...
Тестовая версия сайта расположена по адресу [home.mradmin.ru](https://home.mradmin.ru/)
